...
var Tiddler = require('./tiddler').Tiddler;
var sys = require('sys');
...
// Convert a date to UTC YYYYMMDDHHMMSS string format
Date.prototype.convertToYYYYMMDDHHMMSS = function()
{
    return this.getUTCFullYear()
        ...
        + zeroPad(this.getUTCSeconds(),2);
};


var TiddlyWiki = function(container)
{
        this.container = container;
        this.tiddlyweb = http.createClient(this.container.port,
            this.container.host);
        var tiddlers = {};
	...
	this.clear = function() {
        ...
        this._collectionURI = function() {
            return this.container.base + '/tiddlers';
        };
        this._tiddlerURI = function(title) {
            return this._collectionURI() + '/' + encodeURIComponent(title);
        };
        this._getTiddler = function(title) {
            var emitter = new Emitter();
            var uri = this._tiddlerURI(title);
            sys.puts('GET ' + uri);
            var request = this.tiddlyweb.request('GET', uri, {
        ...
        this._putTiddler = function(tiddler) {
            var emitter = new Emitter();
            var uri = this._tiddlerURI(tiddler.title);
            sys.puts('PUT ' + uri);
            tiddler.modified = tiddler.modified.convertToYYYYMMDDHHMMSS();
            tiddler.created = tiddler.created.convertToYYYYMMDDHHMMSS();
            var tiddlerJSON = JSON.stringify(tiddler);
            var request = this.tiddlyweb.request('PUT', uri, {
                host: this.container.host,
                'content-type': 'application/json',
            });
            request.end(tiddlerJSON);
            request.on('response', function(response) {
        ...
        this._deleteTiddler = function(title) {
            var emitter = new Emitter();
            var uri = this._tiddlerURI(title);
            sys.puts('DELETE ' + uri);
            var request = this.tiddlyweb.request('DELETE', uri, {
                host: this.container.host });
            request.end();
            request.on('response', function(response) {
                if (response.statusCode == '204') {
        ...
        this._listTiddlers = function() {
            var emitter = new Emitter();
            var uri = this._collectionURI();
            sys.puts('LIST ' + uri);
            var request = this.tiddlyweb.request('GET', uri, {
                host: this.container.host,
                accept: 'application/json'});
            ...
            request.on('response', function(response) {
                if (response.statusCode == '200') {
                    ...
                    response.on('end', function() {
                        ...
                        emitter.emit('end', tiddlers);
                    });
                } else {
                    emitter.emit('error', tiddlers);
                }
            });
            return emitter;
        };
...
// XXX Use the defaults after here, until we don't have to.
